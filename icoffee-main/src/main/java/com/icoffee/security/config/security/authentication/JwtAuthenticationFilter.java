...
import com.icoffee.common.dto.ErrorResponseBodyDto;
...
public class JwtAuthenticationFilter extends GenericFilterBean {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException, AccessDeniedException {
        ...
        if (StringUtils.isNotBlank(token)) {
            ...
            try {
                decodedJWT = jwtTokenProvider.verifyToken(token, secretKey);
            } catch (TokenAuthException e) {
                //捕获token校验失败异常
                httpServletResponse.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
                httpServletResponse.setContentType("application/json;charset:utf-8");

                ErrorResponseBodyDto errorResponseBodyDto =  new ErrorResponseBodyDto(HttpServletResponse.SC_UNAUTHORIZED,"Unauthorized","Unauthorized," + e.getMessage());
        ...
        try {
            ...
            ErrorResponseBodyDto errorResponseBodyDto =  new ErrorResponseBodyDto(HttpServletResponse.SC_FORBIDDEN,"Forbidden","Forbidden," + e.getMessage());
            httpServletResponse.getWriter().write(errorResponseBodyDto.toJson());
            return;
            ...
            httpServletResponse.setStatus(HttpServletResponse.SC_FORBIDDEN);
            httpServletResponse.setContentType("application/json;charset:utf-8");
            ErrorResponseBodyDto errorResponseBodyDto =  new ErrorResponseBodyDto(HttpServletResponse.SC_FORBIDDEN,"Forbidden","Unauthorized," + e.getMessage());
            httpServletResponse.getWriter().write(errorResponseBodyDto.toJson());
            return;
        }
